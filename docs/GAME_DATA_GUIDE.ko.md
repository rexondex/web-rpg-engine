...
출구는 `direction`에 `north`, `east`, `south`, `west` 중 하나를 사용합니다. 대상 맵은 반드시 해당 방향의 바로 옆 격자여야 합니다. 동쪽 경계를 넘으면 대상 맵의 서쪽 경계에 나타나고 Y축 위치가 유지됩니다. 북쪽이나 남쪽으로 이동하면 X축 위치가 유지됩니다. 임의의 `spawnX`, `spawnY`를 출구마다 작성하지 않으므로 방향과 등장 위치가 뒤바뀌지 않습니다.
...
`currencies`에 화폐를 만들고 `shops`에 판매 목록과 구매·판매 가격을 작성합니다. NPC의 `shop`에 상점 ID를 연결하면 대화 종료 후 상점이 열립니다.
...
몬스터의 `boss`를 `true`로 하고 `patterns`를 작성합니다. 현재 `chase`, `dash`, `area` 패턴을 지원하며 `hpBelow`, `cooldownMs`, `power`로 페이즈 조건과 위력을 조정합니다.
