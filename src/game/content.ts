...
export function createInitialSave(config: GameProtocol): SaveData {
  ...
  return { version: 3, contentId: config.game.id, contentVersion: config.game.version, savedAt: new Date().toISOString(), playTime: 0, player: { x: config.player.startX, y: config.player.startY, hp: config.player.baseStats.maxHp, level: 1, xp: 0, mapId: config.player.startMap, inventory: { ...config.player.startingItems } }, world: { quests, defeated: {}, flags: {} } };
}
