...
export class SaveManager {
  ...
  static save(slot: string, data: SaveData): void {
    const serialized = JSON.stringify(data); const envelope: SaveEnvelope = { game: config.game.id, format: 3, data, checksum: checksum(serialized) };
  ...
  static parse(raw: string): SaveData {
    ...
    if (parsed.data.version !== 3) return this.migrateLegacy(parsed.data as unknown as Record<string, unknown>);
